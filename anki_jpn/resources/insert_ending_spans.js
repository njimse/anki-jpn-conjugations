function insert_ending_spans(dict_form, conjugation) {
    ...
    if (open_ruby_tags == close_ruby_tags) {
        ...
        new_conjugation = conjugation.slice(0,split_index) + "<span id=ending>" + conjugation.slice(split_index) + "</span>";
        ...
        var new_base = conjugation.slice(0, split_index) + "<span id=ending>" + conjugation.slice(split_index, split_index + rt_close_offset) + "</span>";
        var ruby_close_offset = conjugation.slice(split_index+rt_close_offset).indexOf("</ruby>") + "</ruby>".length;
        var new_ending = conjugation.slice(split_index + rt_close_offset, split_index+rt_close_offset + ruby_close_offset) + "<span id=ending>" + conjugation.slice(split_index+rt_close_offset + ruby_close_offset) + "</span>";
