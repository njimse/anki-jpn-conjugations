...
from aqt import mw # pylint: disable=E0401
from aqt.forms.taglimit import Ui_Dialog # pylint: disable=E0401
from aqt.utils import showInfo, Qt, disable_help_button, restoreGeom, saveGeom, showWarning, tr # pylint: disable=E0401
from aqt.filtered_deck import FilteredDeckConfigDialog # pylint: disable=E0401
from aqt.qt import ( # pylint: disable=E0401
    QMenu, QItemSelectionModel, QDialog, QVBoxLayout, QLabel,
    QWidget, QListWidget, QListWidgetItem, QDialogButtonBox,
    QShortcut, qconnect, QKeySequence
)
...
def custom_choose_list(msg, choices, startrow=0):
...
def select_deck(msg):
    ...
    deck_choice = custom_choose_list(msg, all_decks)
...
def select_tag(msg):
    ...
    tag_choice = custom_choose_list(msg, all_tags)
...
def get_relevant_model_fields(model_name):
    ...
    expression_index = custom_choose_list(
        f"For the '{model_name}' note type, Which field contains the expression?", field_names)
    if expression_index is None:
        return None, None, None
    meaning_index = custom_choose_list(
        f"For the '{model_name}' note type, Which field contains the meaning?", field_names)
    if meaning_index is None:
        return None, None, None
    reading_index = custom_choose_list(
        f"For the '{model_name}' note type, Which field contains the reading?", field_names)
...
def update_adjectives():
    target_deck_id, _ = select_deck("Which deck would you like to update?")
    if target_deck_id is None:
        return
    source_deck_id, source_deck_name = select_deck(
        "Which deck should be used as the source content?")
    ...
    if config.adjective_tags_empty(source_deck_name):
        ...
        if config.adjective_tags_empty(source_deck_name):
            showInfo("At least one tag must be specified for "
                     "i-adjectives, na-adjectives, or general adjectives")
            return
    ...
    for model_name in relevant_models:
        if config.model_fields_empty(model_name):
            ...
            if any(not f for f in relevant_fields):
                showInfo("Expression, meaning, and reading fields " \
                         + "must be specified for all relevant note types")
...
def update_verbs():
    ...
    if config.verb_tags_empty(source_deck_name):
        ...
        if config.verb_tags_empty(source_deck_name):
            showInfo("At least one tag must be specified for " \
                     + "ichidan, godan, irregular, or general adjectives")
            return
    ...
    for model_name in relevant_models:
        if config.model_fields_empty(model_name):
            relevant_fields = get_relevant_model_fields(model_name)
            if any(not f for f in relevant_fields):
                showInfo("Expression, meaning, and reading fields must " \
                         + "be specified for all relevant note types")
            config.add_model_fields(model_name, *relevant_fields)
...
def create_filtered_deck():
    ...
    class CardLimit(QDialog):
        def __init__(
            ...
            """Ask user to select tags. on_success() will be called with
            selected included and excluded tags."""
    ...
    target_deck_id, target_deck_name = select_deck(
        "Which deck has the conjugation notes to be filtered?")
    if target_deck_id is None:
        return
    ...
    if len(cl.include_list) > 1:
        include_query = "(" + " OR ".join(
            f'"card:{card_name}"' for card_name in cl.include_list) + ")"
    ...
    if len(cl.exclude_list) > 1:
        exclude_query = "-(" + " OR ".join(
            f'"card:{card_name}"' for card_name in cl.exclude_list) + ")"
